...
import { GuideList } from '@app/shared/models/guide.interface';
import { GuideComponent } from '../guide/guide.component';
import { GuidedetailComponent } from '../guidedetail/guidedetail.component';
...
@Component({
  selector: 'app-guideedit',
  templateUrl: './guideedit.component.html',
  styleUrls: ['./guideedit.component.scss']
})
export class GuideeditComponent implements OnInit {
  ...
  List(): void {
    ...
    this.guideSvc.GetGuideActivateUser(this.data.idtravel).subscribe(
      guide=>{
        if(guide.length > 0){
  ...
  onOpenModalDetail(guide = {}): void {
    let dialogRef = this.dialog.open(GuidedetailComponent, {
      maxWidth: '100vw',
      maxHeight: '100vh',
      height: '95%',
      width: '95%',
      hasBackdrop: true,
      data: { title: 'Detail', data: this.data, guide},
    });
